...
def k_random_walks(k, treatment, outcome, df, desired_ate, size_threshold, weights_optimization_method):
    ...
    for walk_idx, random_choice in enumerate(random_choices):
        ...
        for i, ((orig_indices, orig_size), (comp_indices, comp_size)) in enumerate(
                zip(dfs_to_remove_data, dfs_to_remove_data_shira_keren)):
        ...
        for i, (df_to_remove_index, df_to_remove_shape) in enumerate(reversed(dfs_to_remove_data_shira_keren)):
            ...
            if df_to_remove_shape in tuples_removed_num:
                print(
                    f"Combo to remove: {combo_to_remove[i][0]}, ATE already computed, tuples_removed: {df_to_remove_shape}")
                ...
                if len(unique_indices) > max_indices_to_process:
                    print(
                        f"Too many indices ({len(unique_indices)}). Sampling {max_indices_to_process} random indices.")
                    import random
                ...
                if len(unique_indices) > batch_size:
                    ...
                    for batch_start in range(0, len(unique_indices), batch_size):
                        ...
                        print(
                            f"  Processing batch {batch_start // batch_size + 1}: indices {batch_start} to {batch_end - 1}")
                        ipdb.set_trace()
                ...
                print(
                    f"Combo to remove: {combo_to_remove[i][0]}, ATE: {ate}, tuples_removed: {df_to_remove_shape}, diff: {diff}, impact: {impact}")
...
def main(csv_name, attributes_for_apriori, treatment, outcome, desired_ate, k, size_threshold,
         weights_optimization_method):
